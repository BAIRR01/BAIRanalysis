function bidsPlotMEGandECoG (projectDir, subject, session, tasks,runnums, ...
    goodChannelRatio,saveFolder,plotChecks)
%
% bidsPlotMEGandECoG (projectDir, subject, session, [tasks],[runnums], ...
%     [goodChannelRatio],[saveFolder],[plotChecks])
%
% Required input
...
%  Optional Input:
%
%     goodChannelRatio: decimal number indicating which proportion of the
%                           MEG sensors within the area of interest to use
%                           (cutoff by variance)
%                            default: .25
...
%     saveFolder:       Name of folder to save images (string)
%                           default: none (does not save images)
%     plotChecks:       whether to plot sanity checks (boolean)
%
% Example:
...
switch siteModality
    case 'meg'
        ...
        if ~exist('goodChannelRatio', 'var') || isempty(goodChannelRatio)
            goodChannelRatio = .25;
        end
    case 'ecog'
        rawDataFldr = 'ieeg';
...
for ii = 1:length (tasks)
    ...
    % concatenate all runs for this task
    for jj = 1:length(runnums{ii})
        % Get TSV information for each run
    ...
    % plots triggers and a an average timeseries across epocs
    if plotChecks
        plotSanityChecks (epocedData, allTriggers, unit)
    end
    
    % for the sake of making plots easier to see, split experiment
    % conditions in half
    ...
    f = 0:length(t)-1;
    % Plot mean time series and amplitudes averaged across "good" sensors in one plot
...
function sensorsToPlot = findMEGChannels(hdr, task,plotChecks)  
...
switch task
    case {'temporalpattern', 'spatialpattern', 'spatialobject', 'hrf','hrfpattern', 'prf'}
        % Get sensors over the lower back of the head (presumably occipital cortex)
