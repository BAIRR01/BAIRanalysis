...
def upload_to_flywheel(fw, bids_dir, group_name, project_name, subject_to_upload=None):
    ...
    for f_subj in bids_dir.iterdir():
        ...
        elif not f_subj.name.startswith('sub-') and subject_to_upload is None:
            # only upload stimuli (or other) folders when you upload all the subjects
...
def main():
    ...
    parser.add_argument(
        'project',
        help='Project name on flywheel (if it exists, it gets overwritten)')
    parser.add_argument(
        '--group',
        default='bair',
    ...
    lg.info(f'UPLOAD TO FLYWHEEL {args.group}/{args.project}, version {VERSION: 4d}')
